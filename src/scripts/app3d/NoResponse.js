...
export class NoResponse extends Popup {
    constructor() {
        super();
        ...
        cancelBtn.on(RODIN.CONST.GAMEPAD_BUTTON_DOWN, () => {
            this.emit('cancel', new RODIN.RodinEvent(this));
            this.close();
        });
        ...
        this.on('open', () => {
            RODIN.messenger.post('popupopened', {popupName: 'noresponse'});
        });

        this.on('close', () => {
            RODIN.messenger.post('popupclosed', {popupName: 'noresponse'});
        });
    }
